...
glm::mat4x4 screenMatProj;
glm::mat4x4 screenMatView;
glm::mat4x4 screenMatModel;
...
void RenderBuffer()
{
	...
	// AKTYWUJEMY program
	glUseProgram(program[PROGRAM_CHANGED]);


	glUniformMatrix4fv(glGetUniformLocation(program[PROGRAM_CHANGED], "screenMatProj"), 1, GL_FALSE, glm::value_ptr(screenMatProj));
	glUniformMatrix4fv(glGetUniformLocation(program[PROGRAM_CHANGED], "screenMatView"), 1, GL_FALSE, glm::value_ptr(screenMatView));
	glUniformMatrix4fv(glGetUniformLocation(program[PROGRAM_CHANGED], "screenMatModel"), 1, GL_FALSE, glm::value_ptr(screenMatModel));


	// AKTYWUJEMY tekstury
	glActiveTexture(GL_TEXTURE0);
	glUniform1i(glGetUniformLocation(program[PROGRAM_CHANGED], "tex0"), 0);
...
void DisplayScene()
{
	...
	screenMatView = glm::mat4x4( 1.0 );
	screenMatView = glm::translate(screenMatView, glm::vec3(_scene_translate_x, _scene_translate_y, -5.0f));
	screenMatView = glm::rotate(screenMatView, 1.80f, glm::vec3( 1.0f, 0.0f, 0.0f ) );


    // Macierz modelu
    matModel =  glm::mat4x4( 1.0 );
	screenMatModel = glm::mat4x4(1.0);
	RenderBuffer();
...
void Reshape( int width, int height )
{
	...
	screenMatProj = glm::perspectiveFov(glm::radians(80.0f), (float)width, (float)height, 0.1f, 100.f);
...
void Initialize()
{
	...
	program[PROGRAM_CHANGED] = glCreateProgram();
	glAttachShader(program[PROGRAM_CHANGED], LoadShader(GL_VERTEX_SHADER, "vertex_screen.glsl"));
	glAttachShader(program[PROGRAM_CHANGED], LoadShader(GL_FRAGMENT_SHADER, "fragment_screen.glsl"));
	LinkAndValidateProgram(program[PROGRAM_CHANGED]);
